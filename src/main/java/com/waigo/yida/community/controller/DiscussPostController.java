...
//        content = HtmlUtils.htmlEscape(content);
        ...
        event.setTopic(CommunityConstant.TOPIC_UPDATE_POST).setEntityType(CommunityConstant.DISCUSS_POST).setEntityId(discussPost.getId());
        eventProducer.publishEvent(event);
        return R.create(200, "发布成功~~~").toString();
    }

    @GetMapping("/write.html")
    public String getWritePage(){
        return "/site/discuss-write";
    }
    ...
    public String get(@PathVariable("id") int id, Model model, Page page, HttpServletRequest request) {
        ...
        if (discussPost == null) {
            ...
            Status failure = Status.failure().lineAddAttribute("jumpText", "这篇帖子不存在哦！！！").lineAddAttribute("path", "/index");
        ...
        Status success = Status.success().lineAddAttribute("user", user).lineAddAttribute("discussPost", discussPost).lineAddAttribute("commentList", commentVoList == null ? Collections.EMPTY_LIST : commentVoList);
    ...
    public String like(int entityType, int entityId, int targetUserId) {
        ...
        return R.create(200, "OK").addAttribute("isLike", isLike).addAttribute("likeCount", likeCount).toString();
    }
